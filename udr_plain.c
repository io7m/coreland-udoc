...
rp_tag_contents(struct udoc *ud, struct udr_ctx *rc)
{
  ...
  for (;;) {
    ...
    dfo_constrain(dfo, PAGE_WIDTH, dfo->page_indent + ((part_cur->depth - part_first->depth) * 2));
    dfo_puts3(dfo, part_cur->num_string, " ", part_cur->title);
    dfo_break_line(dfo);
    dfo_constrain(dfo, PAGE_WIDTH, dfo->page_indent - ((part_cur->depth - part_first->depth) * 2));

    if (part_cur->depth < part_first->depth) break;
...
rp_footnotes(struct udoc *ud, struct udr_ctx *rc)
{
  ...
  unsigned long max = ud_oht_size(&ud->footnotes);
  unsigned long ind = 0;
  unsigned long len = 0;
  ...
  dfo_constrain(dfo, PAGE_WIDTH, dfo->page_indent + 2);
  for (ind = 0; ind < max; ++ind) {
    ud_assert(ud_oht_getind(&ud->footnotes, ind, (void *) &u));
    ...
    rtmp.tree_ctx = 0;
    if (!ud_render_node(ud, &rtmp, &u->list->head->next->data.list)) return 0;
...
rp_tag_section(struct udoc *ud, struct udr_ctx *rc)
{
  ...
  dfo_puts(dfo, rc->part->num_string);
  if (rc->part->title)
    dfo_puts2(dfo, " ", rc->part->title);
...
rp_tag_link_ext(struct udoc *ud, struct udr_ctx *rc)
{
  ...
  link = node->next->data.str;
  if (node->next)
    text = (node->next->next) ? node->next->next->data.str : 0;
...
rp_tag_ref(struct udoc *ud, struct udr_ctx *rc)
{
  ...
  dfo_puts3(dfo, "[ref: ", node->next->data.str, "]");
  dfo_break_line(dfo);
  dfo_break_line(dfo);
...
{
  struct dfo_put *dfo = &rc->out->dfo;
  if (!udr_print_file(ud, rc, rc->tree_ctx->state->node->next->data.str, plain_put, dfo))
    return UD_TREE_FAIL;
...
rp_file_init(struct udoc *ud, struct udr_ctx *rc)
{
  ...
  if (ud->render_header)
    if (!udr_print_file(ud, rc, ud->render_header, plain_put, dfo))
      return UD_TREE_FAIL;

  if (rc->part->title) {
    dfo_constrain(dfo, PAGE_WIDTH, 2);
    dfo_puts(dfo, rc->part->title);
    dfo_break_line(dfo);
    dfo_break_line(dfo);
    dfo_constrain(dfo, PAGE_WIDTH, 0);
  }
...
rp_string(struct udoc *ud, struct udr_ctx *rc)
{
  ...
  if (!ud_tag_by_name(tc->state->list->head->data.sym, &tag)) return UD_TREE_OK;
  switch (tag) {
    ...
    case UDOC_TAG_PARA:
    default:
      dfo_puts(dfo, rc->tree_ctx->state->node->data.str);
      break;
  }
  return UD_TREE_OK;
}
...
rp_file_finish(struct udoc *ud, struct udr_ctx *rc)
{
  ...
  if (ud->render_footer)
    if (!udr_print_file(ud, rc, ud->render_footer, plain_put, dfo))
      return UD_TREE_FAIL;
